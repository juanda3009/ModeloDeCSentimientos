...
modeloPLNRegLogNew = pickle.load(open(os.path.join(
    cur_dir, 'modeloIA', 'LogRegression_PLN_classSentimientos_model.sav'), 'rb'))
# cargando el vocabulario
vocabularyNew = pickle.load(
    open(os.path.join(cur_dir, 'modeloIA', 'vocabulary.pkl'), 'rb'))
# creando un nuevo vectorizador con el vocabulario cargado
vectorizerNew = CountVectorizer(
    min_df=0, lowercase=True, vocabulary=vocabularyNew)
...
def f_clasificar(texto):
    ...
    oracion = [texto]
    # convertimos la oración al array del bagword
    x_bag = vectorizerNew.transform(oracion)
